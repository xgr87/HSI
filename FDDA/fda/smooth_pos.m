...
if ndim == 2    
    if size(coef0,2) ~= ncurve || length(size(coef0)) ~= 2
        coef0 = zeros(nbasis,ncurve);
        warning(['Dimensions of coefficient array inconsistent ', ...
                 'with data Y, initial coefficients set to 0.']);
    end
else
    if length(size(coef0)) ~= 3
        coef0 = zeros(nbasis,ncurve,nvar);
        warning(['Dimensions of coefficient array inconsistent ', ...
                 'with data Y, initial coefficients set to 0.']);
    end
    if  size(coef0,2) ~= ncurve || size(coef0,3) ~= nvar
        coef0 = zeros(nbasis,ncurve,nvar);
        warning(['Dimensions of coefficient array inconsistent ', ...
                 'with data Y, initial coefficients set to 0.']);
    end
