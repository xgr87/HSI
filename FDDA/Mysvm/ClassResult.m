...
CR.accuracy(1) = sum(plabel == label)/numel(label)*100;
str = ...
    sprintf('整体分类准确率 = %g%% (%d/%d)',CR.accuracy(1),sum(plabel == label),numel(label));
disp(str)
for i = 1:numel(model.Label)
    ...
    CR.accuracy(i+1) = p/(p+n)*100;
    str = ...
        sprintf('第 %d 类分类准确率 = %g%% (%d/%d)',model.Label(i),CR.accuracy(i+1),p,p+n);
    disp(str);
end
%%
if type == 1
