...
parser.add_argument("-z", "--test_lsample_num_per_class",
                    type=int, default=0.1, help='0.1')
...
# the number of labeled samples per class 5 4 3 2 1
...
test_data = 'datasets/indian_pines.mat'
test_label = 'datasets/indian_pines_gt.mat'

Spectraldata, GroundTruth = utils.load_data(test_data, test_label)

# get train_loader and test_loader


def get_train_test_loader(Spectraldata, GroundTruth, class_num, shot_num_per_class):
    print(Spectraldata.shape)  # (610, 340, 103)
    [nRow, nColumn, nBand] = Spectraldata.shape
    ...
    # G1 = np.copy(G)
    [Row, Column] = np.nonzero(G > -1)  # (10249,) (10249,)
    # print(Row)
    del Spectraldata
    del GroundTruth
    ...
    num_train = np.zeros(num_class, dtype=np.int64)
    num_test = np.zeros(num_class, dtype=np.int64)
    print('labeled number per class:', TEST_LSAMPLE_NUM_PER_CLASS)

    for i in range(num_class+1):
        if i == 0:
            indices = [j for j, x in enumerate(
                Row.ravel().tolist()) if (G[Row[j], Column[j]] == i)]
            ...
            indices = [j for j, x in enumerate(
                Row.ravel().tolist()) if G[Row[j], Column[j]] == i]
            np.random.shuffle(indices)
            nb_val = math.ceil(shot_num_per_class * len(indices))
            train[i] = indices[:nb_val]
            da_train[i] = []
            test[i] = indices[nb_val:]
            num_train[i-1] = len(train[i])
            num_test[i-1] = len(test[i])
    train_indices = []
    test_indices = []
    da_train_indices = []
    for i in range(num_class+1):
    ...
    print('the number of train samples per class:', num_train)  # 520
    print('the number of test samples per class:', num_test)  # 9729

    nTrain = len(train_indices)
    nTest = len(test_indices)

    imdb = {}
    imdb['data'] = np.zeros([nBand, nTrain + nTest],
    ...
    for iSample in range(nTrain + nTest):
        imdb['data'][:, iSample] = data[Row[RandPerm[iSample]],
...
def get_target_dataset(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    train_loader, test_loader, imdb_da_train, G, RandPerm, Row, Column, nTrain = get_train_test_loader(Spectraldata=Spectraldata,  GroundTruth=GroundTruth,
    ...
    print(imdb_da_train['data'].shape)  # (9, 9, 100, 225)
    print(imdb_da_train['Labels'])
    target_dataset = utils.matcifar(
        imdb_da_train, train=True, d=3, medicinal=0)
    target_loader = torch.utils.data.DataLoader(
        target_dataset, batch_size=128, shuffle=True, num_workers=0)
    del target_dataset

    return train_loader, test_loader, target_da_metatrain_data, target_loader, G, RandPerm, Row, Column, nTrain

if CLASS_BALANCING:
    weights = compute_imf_weights(train_gt, num_class, IGNORED_LABELS)
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Linear(
            in_features=self.final_feat_dim, out_features=CLASS_NUM)
...
for iDataSet in range(nDataSet):
    ...
    train_loader, test_loader, target_da_metatrain_data, target_loader, G,RandPerm,Row, Column,nTrain = get_target_dataset(
        Data_Band_Scaler=Spectraldata, GroundTruth=GroundTruth, class_num=TEST_CLASS_NUM, shot_num_per_class=TEST_LSAMPLE_NUM_PER_CLASS)
    # model
    ...
    feature_encoder_optim = torch.optim.Adam(
        feature_encoder.parameters(), lr=args.learning_rate)
    domain_classifier_optim = torch.optim.Adam(
        domain_classifier.parameters(), lr=args.learning_rate)
    ...
    for episode in range(10000):  # EPISODE = 90000
        ...
        if episode % 2 == 0:
            ...
            task = utils.Task(metatrain_data, CLASS_NUM,
                              SHOT_NUM_PER_CLASS, QUERY_NUM_PER_CLASS)  # 5， 1，15
            support_dataloader = utils.get_HBKC_data_loader(
                task, num_per_class=SHOT_NUM_PER_CLASS, split="train", shuffle=False)
            query_dataloader = utils.get_HBKC_data_loader(
                task, num_per_class=QUERY_NUM_PER_CLASS, split="test", shuffle=True)
            ...
            support_features, support_outputs = feature_encoder(
                supports)  # torch.Size([409, 32, 7, 3, 3])
            query_features, query_outputs = feature_encoder(
                querys)  # torch.Size([409, 32, 7, 3, 3])
            target_features, target_outputs = feature_encoder(
                target_data, domain='target')  # torch.Size([409, 32, 7, 3, 3])

            # Prototype network
            if SHOT_NUM_PER_CLASS > 1:
                support_proto = support_features.reshape(
                    CLASS_NUM, SHOT_NUM_PER_CLASS, -1).mean(dim=1)  # (9, 160)
            else:
            ...
            f_loss = crossEntropy(logits, query_labels.long())

            '''domain adaptation'''
            # calculate domain adaptation loss
            features = torch.cat(
                [support_features, query_features, target_features], dim=0)
            outputs = torch.cat(
                (support_outputs, query_outputs, target_outputs), dim=0)
            ...
            domain_label = torch.zeros(
                [supports.shape[0] + querys.shape[0] + target_data.shape[0], 1])
            # torch.Size([225=9*20+9*4, 100, 9, 9])
            ...
            total_hit += torch.sum(torch.argmax(logits,
                                   dim=1).cpu() == query_labels).item()
            total_num += querys.shape[0]
        # target domain few-shot + domain adaptation
        else:
            '''Few-shot classification for target domain data set'''
            # get few-shot classification samples
            task = utils.Task(target_da_metatrain_data, TEST_CLASS_NUM,
                              SHOT_NUM_PER_CLASS, QUERY_NUM_PER_CLASS)  # 5， 1，15
            support_dataloader = utils.get_HBKC_data_loader(
                task, num_per_class=SHOT_NUM_PER_CLASS, split="train", shuffle=False)
            query_dataloader = utils.get_HBKC_data_loader(
            ...
            support_features, support_outputs = feature_encoder(
                supports,  domain='target')  # torch.Size([409, 32, 7, 3, 3])
            query_features, query_outputs = feature_encoder(
                querys, domain='target')  # torch.Size([409, 32, 7, 3, 3])
            source_features, source_outputs = feature_encoder(
                source_data)  # torch.Size([409, 32, 7, 3, 3])
            ...
            '''domain adaptation'''
            features = torch.cat(
                [support_features, query_features, source_features], dim=0)
            outputs = torch.cat(
                (support_outputs, query_outputs, source_outputs), dim=0)
            softmax_output = nn.Softmax(dim=1)(outputs)

            # domain_label = torch.zeros([supports.shape[0] + querys.shape[0] + source_features.shape[0], 1]).cuda()
            domain_label = torch.zeros(
                [supports.shape[0] + querys.shape[0] + source_features.shape[0], 1])
            # torch.Size([225=9*20+9*4, 100, 9, 9])
            domain_label[supports.shape[0] + querys.shape[0]:] = 1
            ...
            domain_logits = domain_classifier(
                randomlayer_out, episode)  # , label_logits
            domain_loss = domain_criterion(domain_logits, domain_label)
        ...
        if (episode + 1) % 1000 == 0 or episode == 0:
            ...
            train_features, _ = feature_encoder(
                Variable(train_datas), domain='target')  # (45, 160)

            max_value = train_features.max()  # 89.67885
            min_value = train_features.min()  # -57.92479
            print(max_value.item())
            print(min_value.item())
            train_features = (train_features - min_value) * \
                              1.0 / (max_value - min_value)

            KNN_classifier = KNeighborsClassifier(n_neighbors=1)
            KNN_classifier.fit(train_features.cpu().detach(
            ...
            for test_datas, test_labels in test_loader:
                ...
                test_features, _ = feature_encoder(
                    Variable(test_datas), domain='target')  # (100, 160)
                test_features = (test_features - min_value) * \
                                 1.0 / (max_value - min_value)
                predict_labels = KNN_classifier.predict(
                    test_features.cpu().detach().numpy())
            ...
            test_accuracy = 100. * total_rewards / counter
            print('\t\tAccuracy: {}/{} ({:.2f}%)\n'.format(total_rewards, counter,
                                                            100. * total_rewards / counter))
            ...
            if test_accuracy > last_accuracy:
                ...
                acc[iDataSet] = 100. * total_rewards / counter
                OA = acc
                C = metrics.confusion_matrix(labels[posi], predict[posi])
                A[iDataSet, :] = np.diag(C) / np.sum(C, 1, dtype=float)

                k[iDataSet] = metrics.cohen_kappa_score(
                    labels[posi], predict[posi])

            print('best episode:[{}], best accuracy={}'.format(
                best_episdoe + 1, last_accuracy))

    if test_accuracy > best_acc_all:
        best_predict_all = predict
        best_G, best_RandPerm,best_Row, best_Column,best_nTrain = G, RandPerm, Row, Column, nTrain
    print('iter:{} best episode:[{}], best accuracy={}'.format(
        iDataSet, best_episdoe + 1, last_accuracy))
    print('***********************************************************************************')
...
for i in range(len(best_predict_all)):  # predict ndarray <class 'tuple'>: (9729,)
    best_G[best_Row[best_RandPerm[best_nTrain + i]]
        ][best_Column[best_RandPerm[best_nTrain + i]]] = best_predict_all[i] + 1
...
results["predictlabel"] = best_predict_all
best_G1 = best_G[4:-4, 4:-4]
results["wholemap"] = best_G1

savemat('indian_DCFSL.mat', results)
