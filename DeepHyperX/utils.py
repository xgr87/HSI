...
def convert_to_color_(arr_2d, palette=None):
    ...
    if palette is None:
        ...
        palette = {i: np.random.randint(0, 255, 3) for i in unique}
        print("Generated default palette:", palette)
...
def metrics(prediction, target, ignored_labels=[], n_classes=None):
    ...
    valid_labels = [l for l in np.unique(target) if l not in ignored_labels]
    n_classes = max(valid_labels) + 1 if n_classes is None else n_classes
    ...
    for i in range(len(cm)):
        try:
            F1 = 2. * cm[i, i] / (np.sum(cm[i, :]) + np.sum(cm[:, i]) + 0.000001)
            Ap = cm[i, i] / (np.sum(cm[i, :]) + 0.000001)
    ...
    results["Accuracy per class"] = Apc
    results["AA"] = np.sum(Apc[1:])/len(Apc[1:])
...
def show_results(results, vis, label_values=None, agregated=False):
    ...
    if agregated:
        ...
        apc = [r["Accuracy per class"] for r in results]
        F1_scores = [r["F1 scores"] for r in results]
        
        apc_mean = np.mean(apc, axis=0)
        apc_std = np.std(apc, axis=0)
        ...
        aa = results["AA"]
        kappa = results["Kappa"]
        apc = results["Accuracy per class"]
    ...
    if agregated:
        for label, score, std in zip(label_values[1:], apc_mean,
                                     apc_std):
            text += "\t{}: {:.02f} +- {:.02f}\n".format(label, score, std)
    else:
        for label, score in zip(label_values[1:], apc):
            text += "\t{}: {:.02f}\n".format(label, score)
    text += "---\n"
...
def sample_gt(gt, train_size, mode='random'):
    ...
    if mode == 'random':
        indices = np.nonzero(gt)
        if len(indices[0]) == 0:
            raise ValueError("Empty ground truth matrix")

        train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, stratify=y)
        # 解压坐标元组为行和列数组
        train_rows, train_cols = zip(*train_indices) if train_indices else ([], [])
        test_rows, test_cols = zip(*test_indices) if test_indices else ([], [])
        ...
        for c in np.unique(gt):
           ...
           train, test = sklearn.model_selection.train_test_split(X, train_size=train_size[c-1])
           print('Preparing training samples -- ' + str(c) + ' - ' + str(train_size[c-1]))
           train_indices += train
           test_indices += test
    ...
    #    train_indices, test_indices = [], []
    #    for c in np.unique(gt):
    #        train, test, indices, X = [], [], [], []
    #        if c == 0:
    #           continue
    #        indices = np.nonzero(gt == c)
    #        X = list(zip(*indices)) # x,y features
           
    #     #   print('number of sampes for the {} class:{}'.format(c, len(indices[0])))
    #     #    if len(indices[0]) > train_size:
    #     #        train_size1 = train_size  
    #     #    else:
    #     #        train_size1 = 15   # 如果小于 train_size，设置 train_size 为 15
    #        train, test = sklearn.model_selection.train_test_split(X, train_size=train_size)
    #        train_indices += train
    #        test_indices += test
    #    train_indices = [list(t) for t in zip(*train_indices)]
    #    for i, j in zip(train_indices[0], train_indices[1]):
    #        train_gt[i, j] = gt[i, j]
    #    test_indices = [list(t) for t in zip(*test_indices)]
    #    for i, j in zip(test_indices[0], test_indices[1]):
    #        test_gt[i, j] = gt[i, j]
...
def compute_imf_weights(ground_truth, n_classes=None, ignored_classes=[]):
    ...
    """
    valid_labels = [c for c in np.unique(ground_truth) if c not in ignored_classes]
    n_classes = max(valid_labels) + 1 if n_classes is None else n_classes
