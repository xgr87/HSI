# 首先在下方TERMINAL运行python -m visdom.server 后按住Ctrl键手动打开可视化网页 http://localhost:8097
...
# SAMPLE_PERCENTAGE = [15, 50, 50, 50, 50, 50, 15, 50, 15, 50, 50, 50, 50, 50, 50, 50]
...
for run in range(N_RUNS):
    ...
    if MODEL == "SVM_grid":
        ...
        clf = sklearn.model_selection.GridSearchCV(
            clf, SVM_GRID_PARAMS, verbose=5, n_jobs=1
        )
        ...
        if CLASS_BALANCING:
            ...
            hyperparams["weights"] = torch.from_numpy(weights).to(device=CUDA_DEVICE, 
dtype=torch.float32)
        # Neural network
        model, optimizer, loss, hyperparams = get_model(MODEL, **hyperparams)
        # Split train set in train/val
        train_gt, val_gt = sample_gt(train_gt, 0.95, mode="random")
        # train_gt, val_gt = sample_gt(train_gt, 4, mode="fixed")
        ...
        Testime = Testime2 - Testime1
        print('Test time:', Testime)
        
        prelabel.append(prediction1)
        Testimes.append(Testime)
...
results_dict = {
    "Confusion_matrix": [r["Confusion matrix"] for r in results],    #matrix
    "F1_scores": [r["F1 scores"] for r in results],
    "OA": [r["Accuracy"] for r in results],
    "Apc": [r["Accuracy per class"] for r in results],
    "AA": [r["AA"] for r in results],
    "Kappa": [r["Kappa"] for r in results],
    'prelabel': prelabel,  # 保存预测向量
    'Testime': Testimes,
    'Totaltime': Tend - Tstart
}
filename = os.path.join("./Datasets/", DATASET + "_" + MODEL + ".mat")
sio.savemat(filename, results_dict)
