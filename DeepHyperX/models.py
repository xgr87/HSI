...
class HuEtAl(nn.Module):
    ...
    def weight_init(m):
        ...
        if isinstance(m, nn.Linear) or isinstance(m, nn.Conv1d):
            ...
            init.uniform_(m.bias, -0.05, 0.05)
    ...
    def __init__(self, input_channels, n_classes, kernel_size=None, pool_size=None):
        ...
        if kernel_size is None:
            # [In our experiments, k1 is better to be [floor](n1/9)]
            kernel_size = math.floor(input_channels / 9)
        ...
        self.conv = nn.Conv1d(1, 20, kernel_size=kernel_size)
...
class LiuEtAl(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(
            ...
            self.fc2_dec_bn(self.fc2_dec(x) + x_pool1.reshape(-1, self.features_sizes[1]))
        )
        x = F.relu(
            # self.fc3_dec_bn(self.fc3_dec(x) + x_conv1.view(-1, self.features_sizes[0]))
            self.fc3_dec_bn(self.fc3_dec(x) + x_conv1.reshape(-1, self.features_sizes[0]))
        )
        x = self.fc4_dec(x)
        return x_classif, x
...
class BoulchEtAl(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(-1, self.features_sizes)
...
def train(
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network", position=0):
        ...
        for batch_idx, (data, target) in tqdm(
            enumerate(data_loader), position=0
        ):
        ...
        with torch.no_grad():
            ...
            for out, pred in zip(output.view(-1), target.view(-1)):
                if out.item() in ignored_labels:
                    ...
                    total += 1
    return accuracy / (total+0.000001)
