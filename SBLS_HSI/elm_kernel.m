function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,xxx] = elm_kernel(P,T, P2,T2, Elm_Type, Regularization_coefficient, Kernel_type, Kernel_para)
...
%%%%%%%%%% Calculate training & testing classification accuracy
   ...
   xxx=[];
    for i = 1 : size(TV.T, 2)
        ...
        xxx=[xxx label_index_actual];
        if label_index_actual~=label_index_expected
            MissClassificationRate_Testing=MissClassificationRate_Testing+1;
        end
    end
%   for i=1:numel(unique(T2))
%       idx=find(T2==i);
%       MissClassificationRate_Testing=0;
%       for j=1:numel(idx)
%           [x, label_index_actual]=max(TY(:,idx(j)));
%           if label_index_actual~=i
%            MissClassificationRate_Testing=MissClassificationRate_Testing+1;
%         end
%       end
%       TestingAccuracy(i)=1-MissClassificationRate_Testing/numel(idx)  
%       
%   end
