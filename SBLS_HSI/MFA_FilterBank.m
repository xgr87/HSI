function V = MFA_FilterBank(InImg,TrnLabels,Tr_idx_R,Tr_idx_C,options) 
% =======INPUT=============
...
%       options.t = 1;
%       W = constructW(im',options);
%       options.PCARatio = 0.99
%       options.ReducedDim = 100;
%       bSuccess = 0
%       while ~bSuccess
%           [V, eigvalue, bSuccess] = OLPP(W, options, im');
%       end
% 
% options.Fisherface = 0;
% options.Regu = 1;
% options.PCARatio=140;
[V, eigvalue] = MFA(TrnLabels', options,im');

     
%       [V, eigvalue] = LSDA(TrnLabels', options,im');
