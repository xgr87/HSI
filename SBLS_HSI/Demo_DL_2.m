...
ftrain=zeros(size(f,3)+size(indian_pines_corrected,3),numel(Tr_idx_R));
for i=1:numel(Tr_idx_R)
    temp11=reshape(f(Tr_idx_R(i),Tr_idx_C(i),:),size(f,3),1);
    temp21=reshape(indian_pines_corrected(Tr_idx_R(i),Tr_idx_C(i),:),size(indian_pines_corrected,3),1);
    temp11=temp11/norm(temp11);
    temp21=temp21/norm(temp21);
    ftrain(:,i)=[temp11;temp21];
...
ftest=zeros(size(ft,3)+size(indian_pines_corrected,3),numel(Te_idx_R));
for i=1:numel(Te_idx_R)
    temp12=reshape(ft(Te_idx_R(i),Te_idx_C(i),:),size(f,3),1);
    temp22=reshape(indian_pines_corrected(Te_idx_R(i),Te_idx_C(i),:),size(indian_pines_corrected,3),1);
    temp12=temp12/norm(temp12);
    temp22=temp22/norm(temp22);
    ftest(:,i)=[temp12;temp22];
    
    
%     ftest(:,i)=[reshape(ft(Te_idx_R(i),Te_idx_C(i),:),size(ft,3),1); reshape(indian_pines_corrected(Te_idx_R(i),Te_idx_C(i),:),size(indian_pines_corrected,3),1)];
end
