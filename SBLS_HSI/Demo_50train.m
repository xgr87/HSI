...
for i=1:numel(Tr_idx_C)
    TrnData(:,i) = reshape(indian_pines_corrected(Tr_idx_R(i),Tr_idx_R(i),:),200,1); % 
end
...
% clear TrnData; 
tic;
[ftrain,V] = PCANet_train(TrnData,indian_pines_corrected,Tr_idx_R,Tr_idx_C,PCANet,1); % BlkIdx serves the purpose of learning block-wise DR projection matrix; e.g., WPCA
PCANet_TrnTime = toc;
...
tic; 
ftest = PCANet_FeaExt_new(indian_pines_corrected,Te_idx_R,Te_idx_C,V,PCANet);
% for i=1:size(ftest,2)
%     ftest(:,i)=ftest(:,i)/norm(ftest(:,i));
% end

% tic
% [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy(xxx),TY] = elm_kernel(ftrain, TrnLabels,ftest,TestLabels,1, 100, 'RBF_kernel',15);
% toc
end
